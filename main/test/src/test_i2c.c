...
void test_i2c_run(void) {
    ...
    if (err != ESP_OK)
        printf("Slave param config failed\n");
    err = i2c_driver_install(I2C_SLAVE_NUM, slave_conf.mode,
                             I2C_SLAVE_RX_BUF_LEN, I2C_SLAVE_TX_BUF_LEN, 0);
    if (err != ESP_OK)
        printf("Slave driver install failed\n");
    ...
    err = ims_i2c_write(&master_dev, I2C_SLAVE_ADDR, (uint8_t *)send_data,
                        strlen(send_data), 1000);

    if (err != 0) {
        printf("Master write failed: %d\n", err);
    } else {
        int len =
            i2c_slave_read_buffer(I2C_SLAVE_NUM, recv_data, sizeof(recv_data),
