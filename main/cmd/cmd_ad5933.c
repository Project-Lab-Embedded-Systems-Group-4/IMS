...
static int ad5933_dump(int argc, char **argv) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("%d, %d, %d, %.2f, %.2f\n", i, samples[i].real, samples[i].imag, magnitude, impedance);
    }
...
static int ad5933_info(int argc, char **argv) {
    ...
    printf("\tSettling:    %u cycles (multiplier x%d)\n", settle_cycles, 
           (settle_mul == AD5933_SETTLE_X1) ? 1 : (settle_mul == AD5933_SETTLE_X2 ? 2 : 4));
    printf("\tPGA Gain:    %s\n", (ctrl1.pga_gain == AD5933_PGA_GAIN_X1) ? "x1" : "x5");
    
    const char *range_str = "Unknown";
    switch(ctrl1.output_voltage_range) {
        case AD5933_RANGE_2V_PP:    range_str = "2.0V p-p"; break;
        case AD5933_RANGE_200MV_PP: range_str = "200mV p-p"; break;
        case AD5933_RANGE_400MV_PP: range_str = "400mV p-p"; break;
        case AD5933_RANGE_1V_PP:    range_str = "1.0V p-p"; break;
    }
    printf("\tVoltage:     %s\n", range_str);
...
static int ad5933_set_sweep(int argc, char **argv) {
    ...
    const struct ims_device *dev = board_get_device("ad5933");
    if (set_sweep_args.start->count) ad5933_set_start_freq(dev, set_sweep_args.start->ival[0]);
    if (set_sweep_args.inc->count) ad5933_set_inc_freq(dev, set_sweep_args.inc->ival[0]);
    if (set_sweep_args.num->count) ad5933_set_num_inc(dev, (uint16_t)set_sweep_args.num->ival[0]);
...
esp_err_t register_ad5933_command(void) {
    ...
    set_gain_args.gain = arg_dbl1(NULL, NULL, "<g>", "Gain factor (e.g. 5.15e-10)");
    ...
    set_sweep_args.start = arg_int0("s", "start", "<Hz>", "Start frequency in Hz");
    set_sweep_args.inc   = arg_int0("i", "inc", "<Hz>", "Increment frequency in Hz");
    set_sweep_args.num   = arg_int0("n", "num", "<n>", "Number of increments (0-511)");
    ...
    set_pga_args.pga = arg_int1(NULL, NULL, "<0|1>", "PGA Gain: 0 = x5, 1 = x1");
    ...
    set_range_args.range = arg_int1(NULL, NULL, "<0-3>", "Range: 0=2.0V, 1=200mV, 2=400mV, 3=1.0V");
